...
const Sidebar = () => {
  ...
  const navItems=[
    ...
    role==="ADMIN" ? {name:"Employees",href:"/employees",icon:UserIcon}:{name:"Attendance",href:"/attendance",icon:Calendar1Icon},
    ...
    <>
      ...
      <div className='px-5 pt-6 pb-5 border-b border-white/6'>
        <div className='flex items-center justify-between'>
          ...
          <button onClick={(e)=>{setMobileOpen(false)}} className='lg:hidden text-slate-400 hover:text-white p-1'>
      ...
      {/* User Profile Card  */}
        {/* display user profile when username is available */}
  ...
  return (
    <>
      ...
      {/* Mobile overlay -semi transparent layer on entire screen */}
      {mobileOpen && <div className='lg:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-40' onClick={()=>{setMobileOpen(false)}}/>}

      {/* Sidebar desktop  */}
      {/* Sidebar content will be visible only for large screens like desktop */}
      ...
      {/* Sidebar mobile  */}
      {/* this Sidebar content will be visible only for small screens like mobile */}
